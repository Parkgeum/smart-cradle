...
requests.post(SoundURL, data=json.dumps(data), headers=command_headers)
...
while True:
	...
	# channels)
	cam = cv2.VideoCapture('http://223.194.132.29:8090/?action=stream')
	...
	if len(rects)==0:
		...
		requests.post(SERVER+'/uploads/uploads', files=files, headers=headers)
		time.sleep(2)
		count = count + 1
		#print(data)
	...
	for rect in rects:
		...
		#비수면 상태일때(sleep=0)
            ...
            #sleep_start함수가 없으면 잠을 자지 않았다고 판단 print한다
				...
				requests.post(WarnURL, data=json.dumps(data), headers=motor_headers)
